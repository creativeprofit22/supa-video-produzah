...
describe("media-boundary seek quantization", () => {
  it.each([
    ...
    "seeks inside fractional-rate boundary %s without changing canonical input",
    (seconds, expected, frame) => {
      const canonical = Object.freeze({
        frame,
        rate: Object.freeze({ numerator: 30000, denominator: 1001 }),
        seconds,
      });
      ...
      expect(canonical.seconds).toBe(
        (frame * canonical.rate.denominator) / canonical.rate.numerator,
      );
