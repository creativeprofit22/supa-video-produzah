...
test("actual decoded low-tone media: gain, continuous fades, pause/seek and node reuse", async ({
  page,
}, info) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    for (let i = 0; i < samples; i++)
      view.setInt16(
        44 + i * 2,
        Math.round(32767 * 0.04 * Math.sin((2 * Math.PI * 440 * i) / sampleRate)),
        true,
      );
  ...
  await info.attach("preview-audio-rms.json", {
  ...
  for (const point of result.points.filter((p) => p.time > 0.2 && p.time < 3.7)) {
    ...
    expect(Math.abs(point.rms - (0.04 / Math.sqrt(2)) * 10 ** (6 / 20) * envelope)).toBeLessThan(
      0.009,
    );
  }
