...
test("LAYOUT-B01 keyboard persistence and narrow/text reflow", async ({ page }) => {
  ...
  await page.locator(".workspace-layout").evaluate(el => { (el as HTMLElement).style.width = "961px"; });
  await separator.focus();
  await page.keyboard.press("Home");
  const geometry = async () => page.locator(".workbench-grid").evaluate(el => {
    const pane = el.firstElementChild!.getBoundingClientRect();
    return pane.width / (el.getBoundingClientRect().width - 16) * 100;
  });
  await expect.poll(async () => Number(await separator.getAttribute("aria-valuenow"))).toBeCloseTo(288 / 945 * 100, 1);
  ...
  await page.locator(".workspace-layout").evaluate(el => { (el as HTMLElement).style.width = ""; });
  ...
  await page.screenshot({
    path: "../../evidence/2026-09-14-p2-editor-controls/layout-320-text200.png",
    fullPage: true,
  });
});
