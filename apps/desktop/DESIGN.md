...
- `JobCenter`: app-level and project-independent; initial loading, empty, active parent jobs, proxy/thumbnail child stages, cached completion, blocked authorization, retry delay, cancellation pending, cancelled, failed/retryable, complete, restart/database recovery, event disconnect, cache pressure/pinned pressure, legacy empty/present/clearing/failure, and composite-cursor pagination states. Older-page loading preserves the current ledger, rejects duplicate requests, exposes recoverable retry, reports no-more explicitly, and refetches the loaded page depth during live/focus reconciliation. Only valid Cancel/Retry actions are enabled. Restart-blocked final exports instead expose one explicit **Choose destination and retry** action that invokes the native picker, requires the exact persisted destination grant, and resumes the same durable job UUID and render plan.
...
- Job Center uses semantic headings, an ordered parent list, nested child-stage lists, native progress, and one aggregated polite live region that excludes progress ticks. Load older jobs is a native in-flow button below the ledger; it keeps keyboard focus while pending/retry states replace its label and announces only the number of newly merged records. Restart-blocked final exports expose a labeled native button that opens the platform save picker and reports pending, cancelled, failed, queued, and completed states without moving focus.
...
- **Below 480px:** the brand wordmark yields to its accessible icon label; Job Center actions, cache facts, progress, pagination status/action, inspector headings, and dialogs become one column.
...
| Export, collision, cancel, terminal states | Controller, App, component, Rust, and FFmpeg integration tests                                       | Assembled ExportPanel/Job Center cancellation, restart block, native reauthorization, same-job retry, completion, one terminal event, and zero process/partial residue passed |
...
| Job Center and cache lifecycle             | Component/Axe plus Playwright state, focus, reflow, overflow, forced-color, and reduced-motion tests | Assembled restart recovery, active leases, test-budget LRU eviction, identity-stable cache-miss regeneration, and restart-persistent visible legacy confirmation passed       |
...
Phase 2 baseline evidence remains under `apps/desktop/evidence/phase-2/`. Phase 3B evidence under `apps/desktop/evidence/phase-3/` includes responsive/Axe/pagination captures plus literal assembled Windows restart, ExportPanel/Job Center recovery, native picker, cache lease/LRU/cache-miss, and legacy-confirmation captures documented in `assembled-windows-runtime-closure.md`. Desktop TypeScript checks, 106 desktop Vitest tests, 13 Chromium browser tests, 171 non-ignored Rust tests, Axe checks, focus return, residue checks, and literal recovered lifecycle reconciliation pass locally. Field performance, assistive-technology screen-reader output, and external exact-SHA CI remain unverified; no claim is made for them.
